...
- `public/og-default.jpg` — current file is a placeholder generated at build-time from an SVG. Replace with a real photograph (e.g. the east window in morning light) at 1200×630.
- Future routes deferred to a later phase: `/worship/children-and-families` and `/community/get-involved`. The brief's meta-tag table referenced these but no route or page exists yet. When the pages land, add the matching `<Meta>` from the Phase 1 brief.
- `<Navigate>` index routes in `src/App.jsx` (e.g. `/worship` → `/worship/services`) produce SSG pages whose only job is a client-side redirect. The high-traffic ones are already in `vercel.json` `redirects`; consider migrating the rest so the redirect happens at the edge before any HTML is served.

### Pre-rendering (developer)

- The site is statically generated by `vite-react-ssg` at build time. Every route in `src/App.jsx` emits its own HTML file in `dist/` with rendered content and per-page meta tags. The Sanity Studio at `/admin` is the only client-only route and is excluded via `ssgOptions.includedRoutes` in `vite.config.js`.
- Dynamic event and news detail pages get pre-rendered via `getStaticPaths` callbacks in `src/App.jsx` that read slugs from Sanity at build time. If `VITE_SANITY_PROJECT_ID` is unset at build time, those routes are skipped silently and will 404 in production until the next successful build. The trade-off is deliberate: serving the homepage shell under a missing event URL would index that URL with the wrong content.
