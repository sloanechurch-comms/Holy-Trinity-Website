...
- Dynamic event and news detail pages get pre-rendered via `getStaticPaths` callbacks in `src/App.jsx` that read slugs from Sanity at build time. If `VITE_SANITY_PROJECT_ID` is missing at build time, those routes silently skip; `vercel.json` keeps a narrow client-side fallback for them so live URLs still resolve.
